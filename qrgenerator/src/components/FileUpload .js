...
const FileUpload = () => {
  ...
  const handleFileChange = (e) => {
    ...
    // Set the PDF name based on the CSV file name (remove the .csv extension)
    if (uploadedFile) {
      const baseFileName = uploadedFile.name.replace(/\.csv$/, "");
      setPdfName(`${baseFileName}_qr_code.pdf`);
    }
  };
